...
const Navbar = () => {
  ...
  return (
      <nav className={`${open ? 'w-72' : 'w-20'} bg-comp h-screen p-5 pt-8 sticky top-0 duration-300 border-r-4 border-accent-1`}>
          <img src={control} className={`absolute -right-3 top-9 w-7 border-accent-1 border-2 rounded-full ${!open && 'rotate-180'}`} onClick={() => setOpen(!open)}/>
        ...
        <ul className='pt-6'>
...

          {
            token ? (
            <li>
              <Link to="/profile" className={`text-content flex items-center gap-x-4 p-2 hover:bg-accent-2 rounded-md mt-9`}>
